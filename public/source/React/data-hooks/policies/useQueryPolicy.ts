...
import { POLICY } from 'constants/queryKeys';
...
const useQueryPolicy = ({ session, policyId }: Params) => {
  return useQuery({
    queryKey: [POLICY, session, policyId],
    queryFn: queryPolicy({ sessionId: session, policyId: policyId! }),
    enabled: policyId !== undefined
  });
};

export default useQueryPolicy;
